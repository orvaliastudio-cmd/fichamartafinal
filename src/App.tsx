...
import { Printer, Trash2, Save, FileText, CheckCircle2 } from 'lucide-react';
...
const initialData: FormData = {
  cliente: { dataAtendimento: '', dataNascimento: '', nomeCompleto: '', anotacoes: '' },
...
export default function App() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem('ficha_atendimento_data');
    if (savedData) {
      try {
  ...
  const handleSave = () => {
    localStorage.setItem('ficha_atendimento_data', JSON.stringify(formData));
    setIsSaved(true);
    ...
    <div className="min-h-screen bg-stone-100 py-4 md:py-8 px-2 md:px-4 print:p-0 print:bg-white">
      ...
      <div className="max-w-4xl mx-auto mb-6 flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3">
          <button
            onClick={handleClear}
            className="flex items-center gap-2 px-3 md:px-4 py-2 bg-white border border-stone-200 rounded-lg text-stone-500 hover:bg-stone-50 hover:text-stone-700 transition-all shadow-sm text-sm md:text-base active:scale-95"
          >
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-2 px-3 md:px-4 py-2 bg-white border border-stone-200 rounded-lg text-stone-600 hover:bg-stone-50 transition-all shadow-sm text-sm md:text-base relative active:scale-95"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 md:px-4 py-2 bg-white border border-stone-200 rounded-lg text-stone-600 hover:bg-stone-50 transition-all shadow-sm text-sm md:text-base active:scale-95"
          >
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 px-3 md:px-4 py-2 bg-stone-800 text-white rounded-lg hover:bg-stone-900 transition-all shadow-sm text-sm md:text-base active:scale-95"
          >
      ...
      >
        ...
        <div className="print-page-1 min-h-screen md:min-h-[297mm] flex flex-col">
          ...
          <section className="mb-4">
            <h3 className="text-sm font-bold uppercase tracking-[0.2em] mb-3 flex items-center gap-3 text-stone-400">
              1. CLIENTE
              <div className="flex-grow border-t border-stone-100"></div>
            </h3>
            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Data Atendimento</label>
                <input
                  type="text"
                  value={formData.cliente.dataAtendimento}
                  onChange={(e) => updateField('cliente', 'dataAtendimento', e.target.value)}
                  className="border-b border-stone-200 bg-transparent outline-none py-0.5 focus:border-stone-500 focus:bg-white/50 transition-all text-sm"
                  placeholder="___/___/___"
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Nascimento</label>
                <input
                  type="text"
              ...
              <div className="col-span-2 flex flex-col mt-1">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Nome de Batismo</label>
                <input
                  type="text"
                  value={formData.cliente.nomeCompleto}
                  onChange={(e) => updateField('cliente', 'nomeCompleto', e.target.value)}
                  className="border-b border-stone-300 bg-transparent outline-none py-1 focus:border-stone-600 focus:bg-white/50 transition-all font-serif text-lg italic"
                />
              </div>
              <div className="col-span-2 flex flex-col mt-0">
                <textarea
                  value={formData.cliente.anotacoes}
                  onChange={(e) => updateField('cliente', 'anotacoes', e.target.value)}
                  rows={3}
                  className="w-full bg-transparent border-none outline-none resize-none leading-relaxed focus:bg-white/30 transition-all"
                  style={{ backgroundImage: 'linear-gradient(transparent, transparent 27px, #f1f1f1 27px)', backgroundSize: '100% 28px' }}
                />
              </div>
            </div>
          </section>
          ...
          <section className="mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Alma</label>
                <input
                  type="text"
                  value={formData.numerologia.alma}
                  onChange={(e) => updateField('numerologia', 'alma', e.target.value)}
                  className="border-b border-stone-200 bg-transparent outline-none py-0.5 focus:border-stone-400 focus:bg-white/50 transition-all font-serif italic text-base"
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Personalidade</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Destino</label>
                <input
                  type="text"
          ...
          <section className="mb-4">
            ...
            <div className="border border-stone-100 p-3 rounded-sm bg-white/50">
              <div className="mb-2">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest block mb-1">Lição de Vida</label>
                <textarea
                  value={formData.leitura.licaoDeVida}
                  onChange={(e) => updateField('leitura', 'licaoDeVida', e.target.value)}
                  rows={4}
                  className="w-full bg-transparent border-none outline-none resize-none leading-relaxed text-sm font-serif italic focus:bg-white/30 transition-all"
                  style={{ backgroundImage: 'linear-gradient(transparent, transparent 23px, #f1f1f1 23px)', backgroundSize: '100% 24px' }}
                />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest">Ano Pessoal</label>
                <input
                  type="text"
                  value={formData.leitura.anoPessoal}
                  onChange={(e) => updateField('leitura', 'anoPessoal', e.target.value)}
                  className="w-20 border-b border-stone-200 bg-transparent outline-none py-0.5 text-center font-serif italic focus:border-stone-400 focus:bg-white/50 transition-all"
                />
              </div>
            </div>
          </section>
        ...
        <div className="print-page-2 min-h-screen md:min-h-[297mm] flex flex-col pt-8 print:pt-12">
          ...
          <section className="mb-6">
            <h3 className="text-sm font-bold uppercase tracking-[0.2em] mb-4 flex items-center gap-3 text-stone-400">
              4. PLANOS DE CONSCIÊNCIA
              <div className="flex-grow border-t border-stone-100"></div>
            </h3>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 pl-0 sm:pl-2">
              {[
                ...
                <div key={plano.label} className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-bold tracking-[0.1em] text-stone-400 uppercase">— {plano.label}</span>
                  <div className="flex gap-4">
                    {plano.nums.map((num) => (
                      ...
                      >
                        <span className="text-xs font-serif italic text-stone-600">[{num}]</span>
                        <div className={`w-4 h-4 border border-stone-300 flex items-center justify-center rounded-full transition-colors group-hover:border-stone-400 ${formData.planosConsciencia[num - 1] ? 'bg-stone-800 border-stone-800' : ''}`}>
                          {formData.planosConsciencia[num - 1] && <div className="w-1 h-1 bg-white rounded-full"></div>}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest mb-1">Karma</label>
                <textarea
                  value={formData.karma}
                  onChange={(e) => setFormData(prev => ({ ...prev, karma: e.target.value }))}
                  rows={6}
                  className="w-full bg-transparent border-none outline-none resize-none leading-relaxed font-serif italic text-sm focus:bg-white/30 transition-all"
                  style={{ backgroundImage: 'linear-gradient(transparent, transparent 23px, #f1f1f1 23px)', backgroundSize: '100% 24px' }}
                />
              </div>
              <div className="flex flex-col">
                <label className="text-[10px] uppercase text-stone-400 font-bold tracking-widest mb-1">Darma</label>
                <textarea
          ...
          <section className="mb-4">
            <div className="text-center mb-4">
              <h2 className="text-lg tracking-[0.3em] uppercase font-bold text-stone-400">Pináculos — <span className="italic text-[#8b2b2b]/70">Ciclos de Vida</span></h2>
              <div className="mt-1 w-16 h-0.5 bg-[#8b2b2b]/30 mx-auto"></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={p.key} className="border border-stone-100 p-2.5 rounded-sm bg-white/30">
                  <h4 className="text-[9px] font-bold uppercase tracking-[0.2em] mb-1.5 text-stone-400">{p.label}</h4>
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-transparent border-none outline-none resize-none leading-relaxed text-xs font-serif italic focus:bg-white/30 transition-all"
                    style={{ backgroundImage: 'linear-gradient(transparent, transparent 21px, #f1f1f1 21px)', backgroundSize: '100% 22px' }}
                  />
                </div>
              ))}
            </div>
        ...
        <div className="print-page-3 min-h-screen md:min-h-[297mm] flex flex-col pt-8 print:pt-12">
          ...
          <section className="mb-4">
            <div className="text-center mb-6">
              <h2 className="text-lg tracking-[0.3em] uppercase font-bold text-stone-400">Análise Energética — <span className="italic text-[#8b2b2b]/70">Chakras</span></h2>
              <div className="mt-1 w-16 h-0.5 bg-[#8b2b2b]/30 mx-auto"></div>
            </div>
            ...
            <div className="space-y-4 print:space-y-3">
              {[
                ...
                <div key={chakra.key} className="flex items-start gap-4 print:gap-4">
                  ...
                  <div className="flex-grow pt-0 w-full">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-base italic font-bold font-serif text-stone-700">{chakra.label}</span>
                      <div className="flex gap-4">
                        <label className="flex items-center gap-1.5 text-[9px] uppercase cursor-pointer font-bold text-stone-400">
                          <input
                            type="checkbox"
                    ...
                    <div className="flex items-baseline gap-2">
                      <input
                        ...
                        className="flex-grow border-b border-stone-100 bg-transparent outline-none py-0 text-sm font-serif italic focus:border-stone-300 focus:bg-white/50 transition-all"
            ...
            <div className="mt-8 text-center text-[9px] uppercase tracking-[0.3em] text-stone-300 font-bold">
              Análise Energética — Confidencial
            </div>
          </section>
        </div>
      </motion.div>
